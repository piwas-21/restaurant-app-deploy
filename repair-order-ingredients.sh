...
#   ./repair-order-ingredients.sh --data <file> --apply
#   ./repair-order-ingredients.sh --rollback --confirm
#
...
#
# Run ON THE BOX from /opt/rumi/deploy. See DEPLOYMENT.md "Repairing historic order
# ingredient text".
set -euo pipefail
cd "$(dirname "$0")"
...
if [[ -n "${PSQL_CMD:-}" ]]; then
  ...
  PGUSER_BOX="$(grep -E '^POSTGRES_USER=' .env | cut -d= -f2- | tr -d "\"'")"
...
if $ROLLBACK; then
  ...
  echo "deleted $n row(s); $left remain"
...
awk -F'\t' '
  ...
  $3 == ""               { printf("line %d: empty ingredient name\n", NR); bad=1 }
