...
- Changes ship as PRs through the review-gate (Stop + pre-push hooks). **No `--no-verify`, no bypasses.** Deploy-user sudo stays scoped to `chown -h` — never `NOPASSWD:ALL`. **GitFlow — branch + PR always, never commit to `main` or `develop`:** `develop` is the default/integration branch (open `feature/fix/chore/docs` PRs to it); `main` = releases only via a `develop`→`main` PR (a push to `main` rsyncs to the boxes via `sync-to-box.yml` (prod) + `sync-to-staging.yml` (staging)). Both branches are protected by the no-bypass `main-develop` ruleset + a pre-commit `no-commit-to-branch` guard (`.pre-commit-config.yaml`; `bash setup_hooks.sh`, commit-stage only so it never clobbers the pre-push gate). Lint/scan CI runs on every PR (`.github/workflows/ci.yml`): **shellcheck**, **compose config validate**, **trivy config**, and — since 2026-08-18 — **`tests/*.sh` script unit tests**, all blocking. That last job exists because a real provision failed on something no linter can see: the generated admin bootstrap password broke the BACKEND's password policy, a rule living in another repo's C#. If you add a script whose correctness depends on a contract in another repo, test it here.
