...
- Changes ship as PRs through the review-gate (Stop + pre-push hooks). **No `--no-verify`, no bypasses.** Deploy-user sudo stays scoped to `chown -h` — never `NOPASSWD:ALL`. **Branch + PR always — never commit to `main`:** a pre-commit `no-commit-to-branch` guard now blocks it (`.pre-commit-config.yaml`; install once with `bash setup_hooks.sh`, commit-stage only so it never clobbers the pre-push gate). Lint/scan CI runs on every PR (`.github/workflows/ci.yml`).
